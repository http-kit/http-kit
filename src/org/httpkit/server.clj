...
(defn run-server
  ...
  * See http://http-kit.org/migration.html for differences."
  [handler {:keys [port thread ip max-body max-line worker-name-prefix queue-size max-ws socket-idle-timeout]
            :or   {ip                 "0.0.0.0"  ; which ip (if has many ips) to bind
                   ...
                   socket-idle-timeout 0    ; socket idle timeout in millis: 0ms (disabled)
                   }
            }]
  (let [h (RingHandler. thread handler worker-name-prefix queue-size)
        s (HttpServer. ip port h max-body max-line max-ws socket-idle-timeout)]
    (.start s)
